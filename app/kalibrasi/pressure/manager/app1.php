                ...
                if (is_null($img)) {
                    ...
                    try {
                        ...
                        foreach ($tables as $table) {
                            $sql_update = "UPDATE $table SET app3 = ?, manager = ? WHERE id IN ($idsString)";
                        ...
                        header("Location: http://10.11.11.199/dokument-control/app/kalibrasi/pressure/manager/belumapprove.php?username=" . urlencode($username) . "&bagian=" . urlencode($bagian));
                        exit();
                    } catch (Exception $e) {
                        $conn->rollback();
